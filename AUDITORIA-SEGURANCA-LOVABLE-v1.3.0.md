...
> **Changelog v1.3.0:** Fluxo anti-dúvida para usuários leigos; instrução para o Lovable escolher recomendações padrão seguras; relatório com plano de ação claro sem perguntas abertas; matriz de decisões por tipo de projeto
> **Changelog v1.2.2:** Instrução operacional no topo para o Lovable executar a auditoria automaticamente ao receber o `.md`; orientação de uso simplificada para upload direto
...
# 🚨 INSTRUÇÃO OPERACIONAL PARA O LOVABLE

> **Se você é o Lovable lendo este arquivo anexado em um projeto, execute esta instrução automaticamente.**

Ao receber este `.md` anexado dentro de um projeto Lovable, use este documento como referência principal e **rode uma auditoria de segurança do projeto atual**.

Não pergunte genericamente “o que você quer fazer com este arquivo” se o contexto for um projeto Lovable. A ação esperada é:

**Rodar a auditoria de segurança usando este checklist.**

## Objetivo imediato

Gerar um **relatório de auditoria de segurança** do projeto atual, comparando o que já existe no código, banco, Edge Functions, Storage e configurações visíveis com os critérios deste documento.

## Regras obrigatórias para o Lovable

1. **Não altere código automaticamente neste primeiro passo.**
2. **Não recrie sistemas de segurança que já existem.**
3. **Não invente configurações que não consegue verificar.**
4. **Analise o projeto atual antes de recomendar mudanças.**
5. **Se existir `SECURITY.md`, `SECURITY_BASELINE.md`, documentação de segurança ou arquivos semelhantes, compare com eles.**
6. **Quando algo depender de Supabase Dashboard, Stripe, hosting, DNS, variáveis de produção ou painel externo, marque como verificação manual.**
7. **Separe claramente o que está OK, o que é parcial, o que é crítico, o que é manual e o que não se aplica.**
8. **Ao final, diga se o projeto está pronto para publicar, não está pronto, ou pode publicar com ressalvas.**
9. **Não termine o relatório com perguntas abertas do tipo “qual opção você quer?”.** Se houver mais de uma solução, escolha uma **recomendação padrão segura**, explique por que ela é a melhor para o contexto e liste alternativas apenas como secundárias.
10. **Preserve a intenção do projeto.** Não crie autenticação, multi-tenant, Stripe, Storage ou arquitetura nova se o projeto não precisa disso.
11. **Escolha sempre a menor correção segura que resolve o risco real.** Evite overengineering.
12. **Se uma correção depender de painel externo** (Supabase externo, Stripe Dashboard, DNS, hosting, variáveis de produção), entregue o SQL, configuração ou passo exato pronto para o usuário executar, sem deixar decisão em aberto.
13. **Se o projeto permitir alterações de código e o usuário pedir correções**, aplique em lotes pequenos, começando por correções não destrutivas e reversíveis.
14. **Nunca esconda risco aceito.** Se uma decisão for aceitável para app pessoal, mas inadequada para app público/comercial, diga isso claramente.


## Formato obrigatório do relatório

Gere o relatório com esta estrutura:

## Relatório de Auditoria de Segurança

### 1. Resumo executivo
- Decisão final: **publicar / não publicar / publicar com ressalvas**
- Top riscos antes de publicar
- Pontos que já estão bem implementados
- Itens que dependem de verificação manual

### 2. Resultado por área
Classifique cada área como:

- ✅ **OK** — já está bem implementado
- ⚠️ **Parcial** — existe algo, mas precisa reforço
- 🔴 **Crítico** — risco importante antes de publicar
- 🧪 **Manual** — precisa verificar fora do Lovable/Supabase Dashboard/Stripe/hosting
- N/A — não se aplica ao projeto

Áreas obrigatórias:

1. RLS
2. Secrets e API keys
3. Auth
...
10. Webhooks
11. CORS e headers
12. Logs e dados sensíveis
13. Dependências
14. Configuração Supabase
15. Teste real anônimo / usuário A / usuário B
16. RPC / SECURITY DEFINER
17. Anti-regressão de RLS
18. IA / prompt injection
19. Backup / incidente
20. Mobile, se aplicável

### 3. Achados detalhados
Para cada problema encontrado, informe:

- Arquivo ou área afetada
- Trecho, comportamento ou configuração problemática
- Risco real
- Severidade: baixa / média / alta / crítica
- Correção recomendada
- Se a correção pode ser feita no código ou depende de verificação manual

### 4. Testes reais recomendados
Liste testes práticos com:
...
- Visitante anônimo tentando chamar Edge Function privada
- Usuário free tentando acessar recurso pago
- URL maliciosa para testar SSRF
- Prompt injection escondido em HTML, PDF, markdown, comentário ou conteúdo coletado
- Tentativa de acessar arquivo de outro usuário no Storage

### 5. Plano de ação sem dúvidas
Separe em:

- Correções críticas antes de publicar
- Melhorias importantes pós-publicação
- Itens manuais para verificar
- Itens que não se aplicam ao projeto

Para cada correção crítica, entregue sempre:

- **Recomendação padrão:** a ação que você recomenda para este projeto
- **Por que esta é a melhor opção:** explicação curta e prática
- **O que será alterado:** arquivos, tabelas, policies, Edge Functions ou configurações
- **Risco de não corrigir:** impacto real
- **Se depende do usuário:** exatamente onde clicar, qual SQL rodar ou qual valor configurar

> Regra anti-dúvida: se existir uma opção claramente mais segura e simples para o contexto, escolha essa opção como recomendação padrão. Não termine com “me diga qual opção você quer” sem orientar qual é a melhor.

## Matriz de decisão automática por tipo de projeto

Use esta matriz para evitar deixar o usuário sem saber o que fazer:

| Tipo de projeto detectado | Decisão padrão recomendada |
|---|---|
| Site público simples, sem login e sem banco | Não criar auth. Revisar headers, forms, dependências, logs e SEO. |
| App pessoal sem login, mas com dados no Supabase | Não criar auth. Deixar leitura pública apenas se fizer sentido. Bloquear INSERT/UPDATE/DELETE para `anon`. Editar dados pelo Dashboard ou por função protegida. |
| App público com dados editáveis | Exigir auth ou mover escrita para Edge Function com autorização clara. Nunca deixar escrita pública por anon. |
| SaaS ou dashboard com usuários | RLS por `user_id`/`tenant_id`, Edge Functions com JWT, prevenção de IDOR e teste usuário A/B. |
| App com IA/LLM | Rate limit, controle de custo, cache, validação de output, proteção contra prompt injection e nunca enviar secrets ao LLM. |
| App que audita/scrapeia URLs | Proteção SSRF em camadas: protocolo, DNS, IP privado, metadata cloud, redirects, timeout e tamanho máximo. |
| App com Stripe/pagamento | Webhook com assinatura, raw body, idempotência e server-side entitlement. |
| App com upload/storage | Buckets privados por padrão, signed URLs, limite de tamanho/tipo e RLS no Storage. |
| App mobile/Capacitor | Sem secrets no bundle, secure storage, deep links seguros e validação server-side de compras. |
| Saúde, jurídico, financeiro ou dados sensíveis | Não publicar sem revisão manual/pentest; aplicar todos os módulos relevantes. |

## Observação sobre correções

A primeira entrega deve ser o relatório completo com recomendação padrão e plano de ação claro.

Se o ambiente do Lovable permitir aplicar correções e o usuário confirmar a execução, aplique em etapas pequenas, priorizando:

1. Vazamento de dados / RLS / IDOR
2. Secrets expostos
3. Webhooks e pagamentos
4. Edge Functions abertas e rate limit
5. SSRF e chamadas externas
6. Logs com PII/secrets
7. Headers/CORS
8. Dependências

Não aplique automaticamente ações destrutivas, como apagar dados, remover tabelas, trocar auth, rotacionar secrets ou alterar billing, sem confirmação explícita.

---

## 👤 Instrução para o usuário humano

O uso principal deste arquivo é simples:

1. Baixe este `.md`.
2. Anexe no projeto Lovable que deseja auditar.
3. O Lovable deve usar a instrução operacional acima para rodar a auditoria, gerar o relatório e entregar uma recomendação padrão clara, sem deixar o usuário escolhendo entre opções técnicas.

Se a interface do Lovable ainda mostrar uma tela de confirmação, escolha a opção equivalente a:

**Rodar a auditoria de segurança**

ou:

**Aplicar o checklist do `.md` neste projeto**

Isso é apenas uma confirmação da interface. O conteúdo deste arquivo já informa ao Lovable o que deve ser feito.

O usuário não precisa entender RLS, Edge Functions, SSRF, webhooks ou CORS para começar. O relatório deve explicar o risco e dizer a ação recomendada para o contexto do projeto.
...
O uso recomendado é **anexar este `.md` diretamente no projeto Lovable** que você quer auditar.

Este arquivo já contém, no topo, uma **instrução operacional para o Lovable** executar a auditoria de segurança automaticamente, gerar um relatório inicial, escolher recomendações padrão seguras e não deixar o usuário preso em decisões técnicas abertas.

Além disso, o documento também está dividido em **3 níveis de uso**:

1. **INSTRUÇÃO OPERACIONAL PARA O LOVABLE** — Deve ser executada quando o arquivo for anexado ao projeto
2. **PROMPT MESTRE** — Pode ser copiado manualmente se o usuário quiser rodar uma auditoria completa pelo chat
3. **19 PROMPTS MODULARES + CHECKLIST MANUAL** — Áreas separadas para aprofundar a revisão ou validar itens fora do Lovable

**Antes de qualquer publicação importante**: clique em **Publish → Review Security** no Lovable. Os scanners nativos (RLS analysis, Database security check, Dependency audit, Code security review) são **gratuitos** e não consomem créditos.
...
> Use este bloco quando preferir copiar e colar manualmente no chat do Lovable. Se você apenas anexou este `.md`, a instrução operacional do topo já diz ao Lovable para iniciar a auditoria.
...
=== REGRA ANTI-DÚVIDA ===
- Não termine com perguntas abertas se houver uma recomendação padrão segura.
- Se houver opções, escolha uma recomendação principal para o contexto do projeto e explique por quê.
- Preserve a intenção do projeto: não crie auth, Stripe, Storage ou multi-tenant se o projeto não precisa.
- Se algo depende de painel externo, entregue o SQL, configuração ou passo exato.
